...
export function AdminDisputes({ onNavigate }: AdminDisputesProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminSidebar currentPage="admin-disputes" onNavigate={onNavigate} />
            ...
            <div className="p-4 md:p-6 border-b border-gray-200">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                />
              </div>
            </div>
              ...
              {disputes.map((dispute, index) => (
                <button
                  ...
                  className={`w-full p-5 border-b border-gray-200 hover:bg-gray-50 transition-colors text-left ${
                    selectedDispute === index ? 'bg-emerald-50 border-l-4 border-l-emerald-600' : 'border-l-4 border-l-transparent'
                  }`}
                >
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-10 h-10 bg-emerald-600 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0" style={{ fontWeight: 600 }}>
                  ...
                  {disputes[selectedDispute].messages.map((msg, idx) => (
                    <div key={idx} className={`flex gap-3 ${msg.isAdmin ? 'justify-end' : ''}`}>
                      ...
                      <div className={`flex-1 max-w-2xl ${msg.isAdmin ? 'text-right' : ''}`}>
                        <div className={`inline-block p-4 rounded-xl ${msg.isAdmin ? 'bg-emerald-600 text-white' : 'bg-white text-gray-900 shadow-sm border border-gray-100'}`}>
